...
export default function Card({
  ...
  const content = (
    <motion.article
      ...
      initial="rest"
      whileHover="hover"
      className={`bg-white rounded-sm overflow-hidden border border-[#DDE3EC] border-t-2 border-t-[#C8972B] flex flex-col h-full ${className}`}
    ...
    >
      <div className="relative aspect-video overflow-hidden bg-[#EFF3F8]">
        {!imgError && image ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-[#EFF3F8] text-[#5A6A7A] text-sm">
            Image not available
          </div>
        )}

        {badge && (
          <span className="absolute top-3 right-3 px-3 py-1 bg-[#1E3A5F] text-[#C8972B] text-xs font-bold rounded-sm uppercase tracking-wider">
            {badge}
          </span>
        )}
      </div>
      ...
      <div className="p-6 sm:p-7 flex flex-col flex-1">
        {Icon && (
          <div className="w-12 h-12 rounded-sm bg-[#1E3A5F]/10 flex items-center justify-center mb-4 shrink-0 text-[#1E3A5F] border border-[#1E3A5F]/20">
